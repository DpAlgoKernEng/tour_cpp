# C 语言 do-while 循环详解
  ...
  [1. do-while 循环基本概念](#1-do-while-循环基本概念)
  
  [2. do-while 循环语法详解](#2-do-while-循环语法详解)
  
  [3. do-while 循环执行机制](#3-do-while-循环执行机制)
  
  [4. do-while 循环与 while 循环对比](#4-do-while-循环与-while-循环对比)
  
  [5. do-while 循环与 for 循环对比](#5-do-while-循环与-for-循环对比)
  
  [6. do-while 循环中的控制语句](#6-do-while-循环中的控制语句)
  
  [7. do-while 循环的作用域特性](#7-do-while-循环的作用域特性)
  
  [8. C23 属性说明符支持](#8-c23-属性说明符支持)
  
  [9. do-while 循环的性能考虑](#9-do-while-循环的性能考虑)
...
在 C 语言中，`do-while` 循环是一种后测试循环（post-test loop），它在每次迭代之后检查控制表达式的值。无论控制表达式的初始值如何，循环体至少会执行一次。如果表达式的值为非零（真），则继续执行循环；如果表达式的值为零（假），则退出循环。
...
### 1.3 do-while 循环与其他控制结构的关系

`do-while` 循环是结构化程序设计三大基本结构之一（顺序、选择、循环）：
...
- `expression` 是一个标量类型的表达式，在每次迭代之后计算。

### 2.2 语法元素详解

**语句体（statement）**：

`do-while` 循环体可以是任何语句，包括：
...
void c23_attributes_example() {
    ...
    printf("C23 attributes support for do-while loops\n");
}
```
...
void do_while_execution_flow() {
    ...
    printf("After do-while loop: x = %d\n", x);
}
```

### 3.2 控制变量的修改

在 `do-while` 循环中，通常需要在循环体内修改控制变量，以确保循环能够正常终止：
...
`do-while` 循环建立了自己的块作用域，其中声明的变量只在该作用域内有效：
...
void block_scope_examples() {
    ...
    do {
        int y = x * 2;  // y 只在 do-while 循环的作用域内有效
        printf("x = %d, y = %d\n", x, y);
        x++;
    } while (x < 3);
...
从 C99 标准开始，可以在 `do-while` 循环的条件中声明变量：
...
C23 标准为 `do-while` 循环引入了属性说明符支持，允许为语句附加元信息：
...
`do-while` 循环相关标准条款的演进：

- C23 标准（ISO/IEC 9899:2024）：6.8.5.2 do 语句，引入属性说明符支持
- C17 标准（ISO/IEC 9899:2018）：6.8.5.2 do 语句（第109页）
- C11 标准（ISO/IEC 9899:2011）：6.8.5.2 do 语句（第151页）
- C99 标准（ISO/IEC 9899:1999）：6.8.5.2 do 语句（第136页）增加了对块作用域的支持
- C89/C90 标准（ISO/IEC 9899:1990）：3.6.5.2 do 语句建立了基本的 do-while 循环框架

### 12.2 相关标准条款详解

6.8.5.2 do 语句（C17 标准）

该条款详细定义了 `do-while` 循环的语法和语义：
...
- 执行机制：先执行循环体，然后计算表达式的值，如果为非零则继续执行循环体
- 作用域：整个 do-while 语句有自己的块作用域
...
- 单一入口单一出口：每个 `do-while` 循环应该有清晰的入口和出口
- 避免无限循环：确保循环条件最终会变为假
- 清晰的控制流：使用适当的缩进和注释
...
C23 标准引入的属性说明符等特性将进一步增强 `do-while` 循环的功能：
...
通过深入理解和熟练掌握 C 语言的 `do-while` 循环，开发者可以编写出更加高效、安全和可维护的代码。`do-while` 循环不仅是程序执行的基本控制结构，更是实现复杂逻辑和算法的基础构建块。随着 C 语言标准的不断演进，新的特性和最佳实践将继续推动现代 C 编程的发展。
